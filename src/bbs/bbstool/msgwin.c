...
rte_display(void)
{
	...
	XmListDeleteAllItems(route_list);
...
grp_display(void)
{
	...
	XmListDeleteAllItems(group_list);
...
msg_create_window(void)
{
	...
	sprintf(title, "Message List @ %s", Bbs_Host);
	...
	sprintf(title, "Route @ %s", Bbs_Host);
	n = 0;
	XtSetArg(args[n], XmNmwmDecorations, Decoration); n++;
	XtSetArg(args[n], XmNtitle, title); n++;
	...
	sprintf(title, "Message @ %s", Bbs_Host);
	n = 0;
	XtSetArg(args[n], XmNmwmDecorations, Decoration); n++;
	XtSetArg(args[n], XmNtitle, title); n++;
